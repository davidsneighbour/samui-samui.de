...
Text inputs and textareas intentionally use the normal 16px body size rather
than the compact helper-text size. This avoids iOS Safari's focused-field zoom
while labels and legal copy can remain visually smaller.

Submit feedback uses the existing `muted`, `border`, `primary`, and `link`
tokens instead of standalone success/error hex colors. The message text carries
the actual result, while the left border gives a compact visual cue without
adding a new semantic color palette.
