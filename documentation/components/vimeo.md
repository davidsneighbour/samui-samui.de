...
* **Click-to-load facade.** On connect, the element fetches only `https://vimeo.com/api/oembed.json` for the video's thumbnail and title — the real `player.vimeo.com` iframe is not created until the user clicks the element (or `autoload` triggers it). This is the entire point of porting lite-vimeo: a page with several embeds doesn't pay for several Vimeo players until the visitor actually asks for one.
