...
Production must allow `https://giscus.app` to fetch these custom theme files. `netlify.toml` therefore keeps `Access-Control-Allow-Origin: https://giscus.app` on `/assets/styles/giscus-samui-*.css` and the webfont assets used by those stylesheets.
