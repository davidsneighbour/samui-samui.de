...
`src/components/ThemeToggle.astro` renders the masthead button that switches between the site's light and dark color schemes.
...
The component script uses one processed module with delegated click handling and an `astro:page-load` listener because the site uses Astro view transitions. That keeps package imports available while still updating the button after a page swap.
