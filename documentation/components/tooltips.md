...
Use the component instead of adding local absolute-positioned tooltip markup. It positions content with `position: fixed`, opens below the trigger, centers to the trigger when there is room, and clamps to the viewport so cards with `overflow-hidden` do not cut it off. The arrow remains aligned to the trigger even when the tooltip body is clamped away from a viewport edge.
