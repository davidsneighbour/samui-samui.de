...
The Astro side uses the Vite watcher for file changes. It uses Astro's normal
logging level by default; run `npm run dev:verbose` when the site server needs
Astro's verbose output and frontmatter debugging.
...
Use `npm run dev:site` when only the Astro site server is needed, and
`npm run dev:site:verbose` for the same server with Astro's verbose output.
